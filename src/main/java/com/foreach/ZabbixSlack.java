...
public class ZabbixSlack {
    private static final String TOKEN = "xoxp-XXXXXXXXXX-XXXXXXXX-XXXXXXXX-XXXXXXXXXXXXXXXXXXXXXXXXXX";
    ...
    private static String idForChannel(SearchableSlackApiClient client, String channel) {
        if (StringUtils.startsWithIgnoreCase(channel, "#zabbix")) {
            ...
            for (Channel c : channelList) {
                if (StringUtils.equalsIgnoreCase(channel.substring( 1 ), c.getName())) {
                    return c.getId();
                }
            }
